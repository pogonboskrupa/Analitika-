...
interface ActualData {
  cTrupci: number; dzgo: number; lTrupci: number; cijepano: number; ukupno: number
}
...
// Odsjeci merged: visoka + izdanačka per GJ+odjel
// DZGO = cjepano četinara (col K), Cijepano = cjepano lišćara (col O)
...
// Proportional split for odjel 66 (only odjel spanning 2 GJ)
// Risovac 2307 m³ / Grmeč 4493 m³ / total 6800 m³
...
const COLORS = { cTrupci:'#1e40af', dzgo:'#7c3aed', lTrupci:'#15803d', cijepano:'#b45309' }
...
function ThSort({ children, col, sort, asc, onSort }: {
  children: React.ReactNode; col: string; sort: string; asc: boolean; onSort:(c:string)=>void
}) {
  return (
    <th onClick={() => onSort(col)}
      className="px-3 py-2.5 text-right text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider cursor-pointer select-none hover:text-gray-600 dark:hover:text-gray-300 whitespace-nowrap">
...
function PoGrupama({ rows, onStatus }: { rows: OdjelRow[]; onStatus:(gj:GJ,o:string,v:StatusOverride)=>void }) {
  ...
  function handleSort(col: string) {
    if (sort===col) setAsc(p=>!p); else { setSort(col); setAsc(false) }
  }
  ...
  const chartData = useMemo(() =>
    [...rows].sort((a,b)=>a.neto-b.neto).slice(-15).map(d=>({
      name: d.odjel,
      'Plan Trupci Č':   d.cTrupci,    'Plan DZGO':       d.dzgo,
      'Plan Trupci L':   d.lTrupci,    'Plan Cijepano':   d.cijepano,
      'Ostvr. Trupci Č': d.actual.cTrupci, 'Ostvr. DZGO': d.actual.dzgo,
      'Ostvr. Trupci L': d.actual.lTrupci, 'Ostvr. Cijepano': d.actual.cijepano,
    }))
  , [rows])

  const fmt0 = (v: number) => v > 0
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm">
        <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-800">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Planirana masa (tamna) vs. ostvarena (svjetla) po skupinama</p>
        </div>
        <div className="p-5">
          <ResponsiveContainer width="100%" height={Math.max(300, chartData.length*32)}>
            <BarChart data={chartData} layout="vertical" margin={{ top:4, right:20, left:10, bottom:4 }}>
              ...
              <Bar dataKey="Plan Trupci Č"   stackId="p" fill={COLORS.cTrupci}  fillOpacity={0.9} maxBarSize={11} />
              <Bar dataKey="Plan DZGO"        stackId="p" fill={COLORS.dzgo}     fillOpacity={0.9} maxBarSize={11} />
              <Bar dataKey="Plan Trupci L"    stackId="p" fill={COLORS.lTrupci}  fillOpacity={0.9} maxBarSize={11} />
              <Bar dataKey="Plan Cijepano"    stackId="p" fill={COLORS.cijepano} fillOpacity={0.9} maxBarSize={11} />
              <Bar dataKey="Ostvr. Trupci Č"  stackId="a" fill={COLORS.cTrupci}  fillOpacity={0.4} maxBarSize={11} />
              <Bar dataKey="Ostvr. DZGO"      stackId="a" fill={COLORS.dzgo}     fillOpacity={0.4} maxBarSize={11} />
              <Bar dataKey="Ostvr. Trupci L"  stackId="a" fill={COLORS.lTrupci}  fillOpacity={0.4} maxBarSize={11} />
              <Bar dataKey="Ostvr. Cijepano"  stackId="a" fill={COLORS.cijepano} fillOpacity={0.4} maxBarSize={11} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm">
        <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-800">
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Detalji po odjelima i GJ</h3>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Koef. = neto/bruto · Status se može ručno podesiti</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-700">
                ...
                <ThSort col="odjel" sort={sort} asc={asc} onSort={handleSort}><span className="text-left">Odjel</span></ThSort>
                <th className="px-3 py-2.5 text-left text-xs font-medium text-gray-400 uppercase tracking-wider whitespace-nowrap">Status</th>
                <ThSort col="koef" sort={sort} asc={asc} onSort={handleSort}>Koef.</ThSort>
                <th colSpan={2} className="px-3 py-2.5 text-center text-xs font-medium uppercase tracking-wider border-l border-gray-200 dark:border-gray-700 whitespace-nowrap" style={{ color:COLORS.cTrupci }}>Trupci Č</th>
                <th colSpan={2} className="px-3 py-2.5 text-center text-xs font-medium uppercase tracking-wider border-l border-gray-200 dark:border-gray-700 whitespace-nowrap" style={{ color:COLORS.dzgo }}>DZGO</th>
                <th colSpan={2} className="px-3 py-2.5 text-center text-xs font-medium uppercase tracking-wider border-l border-gray-200 dark:border-gray-700 whitespace-nowrap" style={{ color:COLORS.lTrupci }}>Trupci L</th>
                <th colSpan={2} className="px-3 py-2.5 text-center text-xs font-medium uppercase tracking-wider border-l border-gray-200 dark:border-gray-700 whitespace-nowrap" style={{ color:COLORS.cijepano }}>Cijepano</th>
                <ThSort col="plan"      sort={sort} asc={asc} onSort={handleSort}>Plan m³</ThSort>
                <ThSort col="ostvareno" sort={sort} asc={asc} onSort={handleSort}>Ostvr. m³</ThSort>
                <ThSort col="stepen"    sort={sort} asc={asc} onSort={handleSort}>Stepen</ThSort>
              </tr>
              <tr className="bg-gray-50/60 dark:bg-gray-800/30 border-b border-gray-200 dark:border-gray-700 text-xs text-gray-400">
                <td /><td /><td /><td />
                {[0,1,2,3].map(i=>(
                  <><td key={`hp${i}`} className={cn('px-3 py-1 text-right',i===0&&'border-l border-gray-200 dark:border-gray-700')}>Plan</td>
                  <td key={`ha${i}`} className="px-3 py-1 text-right">Ostvr.</td></>
                ))}
                <td /><td /><td />
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {GJ_LIST.map(gj => {
                ...
                return (
                  <>
                    <GJHeader key={`h-${gj}`} gj={gj} colSpan={COL} />
                    {gjRows.map((row, i) => (
                      <tr key={`${gj}-${row.odjel}`} className="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                        ...
                        <td className={cn('px-3 py-2 text-right text-xs font-semibold tabular-nums', koefCls(row.koef))}>{row.koef.toFixed(1)}%</td>
                        {/* Trupci Č */}
                        <td className="px-3 py-2 text-right tabular-nums text-gray-500 border-l border-gray-100 dark:border-gray-800 whitespace-nowrap">{formatNumber(row.cTrupci,0)}</td>
                        <td className="px-3 py-2 text-right whitespace-nowrap" style={{ color:COLORS.cTrupci }}>{fmt0(row.actual.cTrupci)}</td>
                        {/* DZGO */}
                        <td className="px-3 py-2 text-right tabular-nums text-gray-500 border-l border-gray-100 dark:border-gray-800 whitespace-nowrap">{formatNumber(row.dzgo,0)}</td>
                        <td className="px-3 py-2 text-right whitespace-nowrap" style={{ color:COLORS.dzgo }}>{fmt0(row.actual.dzgo)}</td>
                        {/* Trupci L */}
                        <td className="px-3 py-2 text-right tabular-nums text-gray-500 border-l border-gray-100 dark:border-gray-800 whitespace-nowrap">{formatNumber(row.lTrupci,0)}</td>
                        <td className="px-3 py-2 text-right whitespace-nowrap" style={{ color:COLORS.lTrupci }}>{fmt0(row.actual.lTrupci)}</td>
                        {/* Cijepano */}
                        <td className="px-3 py-2 text-right tabular-nums text-gray-500 border-l border-gray-100 dark:border-gray-800 whitespace-nowrap">{formatNumber(row.cijepano,0)}</td>
                        <td className="px-3 py-2 text-right whitespace-nowrap" style={{ color:COLORS.cijepano }}>{fmt0(row.actual.cijepano)}</td>
                        {/* Totals */}
                        <td className="px-3 py-2 text-right tabular-nums font-medium text-gray-700 dark:text-gray-200 whitespace-nowrap">{formatNumber(row.neto,0)}</td>
        ...
        <p className="px-5 py-2 text-xs text-gray-400 dark:text-gray-500 border-t border-gray-100 dark:border-gray-800">
          * Odjel 66 se pojavljuje u RK i GJ — ostvarena masa raspoređena proporcionalno (RK 34% / GJ 66%).
        </p>
      </div>
    </div>
  )
...
function PoSortimentima({ rows, onStatus, totals }: {
  ...
  const chartData = useMemo(() =>
    [...rows].filter(d=>d.neto>0).map(d=>({
      ...
      'DZGO':     d.dzgo     >0 ? +(d.actual.dzgo     /d.dzgo     *100).toFixed(1) : 0,
      'Trupci L': d.lTrupci  >0 ? +(d.actual.lTrupci  /d.lTrupci  *100).toFixed(1) : 0,
      'Cijepano': d.cijepano >0 ? +(d.actual.cijepano /d.cijepano *100).toFixed(1) : 0,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <SortimentCard label="Trupci Č" plan={totals.planCT}  actual={totals.actCT}  color={COLORS.cTrupci} />
        <SortimentCard label="DZGO"     plan={totals.planDz}  actual={totals.actDz}  color={COLORS.dzgo} />
        <SortimentCard label="Trupci L" plan={totals.planLT}  actual={totals.actLT}  color={COLORS.lTrupci} />
        <SortimentCard label="Cijepano" plan={totals.planCij} actual={totals.actCij} color={COLORS.cijepano} />
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm p-5">
        ...
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-4">
          {([['Trupci Č',totals.planCT,totals.actCT,COLORS.cTrupci],
             ['DZGO',    totals.planDz,totals.actDz,COLORS.dzgo],
             ['Trupci L',totals.planLT,totals.actLT,COLORS.lTrupci],
             ['Cijepano',totals.planCij,totals.actCij,COLORS.cijepano],
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm">
        <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-800">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">% ostvarenja planiranih masa po sortimentnim grupama · crvena linija = 100%</p>
        </div>
        <div className="p-5">
          <ResponsiveContainer width="100%" height={Math.max(300, chartData.length*34)}>
            <BarChart data={chartData} layout="vertical" margin={{ top:4, right:50, left:10, bottom:4 }}>
              ...
              <Legend wrapperStyle={{ fontSize:'11px', paddingTop:'8px' }} formatter={(v:string)=><span style={{ color:'#6b7280' }}>{v}</span>} />
              <Bar dataKey="Trupci Č" fill={COLORS.cTrupci}  maxBarSize={9} />
              <Bar dataKey="DZGO"     fill={COLORS.dzgo}     maxBarSize={9} />
              <Bar dataKey="Trupci L" fill={COLORS.lTrupci}  maxBarSize={9} />
              <Bar dataKey="Cijepano" fill={COLORS.cijepano} maxBarSize={9} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-700">
                ...
                <ThSort col="ostvareno" sort={sort} asc={asc} onSort={handleSort}>Ostvr. m³</ThSort>
                <ThSort col="stepen"    sort={sort} asc={asc} onSort={handleSort}>Stepen</ThSort>
                <th className="px-3 py-2.5 text-right text-xs font-medium uppercase tracking-wider whitespace-nowrap" style={{ color:COLORS.cTrupci }}>Trupci Č %</th>
                <th className="px-3 py-2.5 text-right text-xs font-medium uppercase tracking-wider whitespace-nowrap" style={{ color:COLORS.dzgo }}>DZGO %</th>
                <th className="px-3 py-2.5 text-right text-xs font-medium uppercase tracking-wider whitespace-nowrap" style={{ color:COLORS.lTrupci }}>Trupci L %</th>
                <th className="px-3 py-2.5 text-right text-xs font-medium uppercase tracking-wider whitespace-nowrap" style={{ color:COLORS.cijepano }}>Cijepano %</th>
              ...
              {sorted.map((row, idx) => {
                ...
                return (
                  <tr key={`${row.gj}-${row.odjel}`} className="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                    ...
                    <td className="px-3 py-2 text-right"><RealizacijaBadge pct={row.stepen} /></td>
                    {(['cTrupci','dzgo','lTrupci','cijepano'] as const).map(k=>{
                      const v = sp(row.actual[k], row[k])
                      return (
                        <td key={k} className="px-3 py-2 text-right whitespace-nowrap">
                          {v!==null&&row[k]>0 ? <RealizacijaBadge pct={v} /> : <span className="text-gray-300 dark:text-gray-700">—</span>}
                        </td>
                      )
                    })}
...
export default function GodišnjiPlan() {
  ...
  const actualByOdjel = useMemo(() => {
    ...
    for (const r of primkaRows) {
      ...
      const e = m.get(key)
      if (e) { e.cTrupci+=cTrupci; e.dzgo+=dzgo; e.lTrupci+=lTrupci; e.cijepano+=cijepano; e.ukupno+=ukupno }
      else   { m.set(key, { cTrupci, dzgo, lTrupci, cijepano, ukupno }) }
    }
    return m
  }, [primkaRows])
  ...
  const allRows = useMemo<OdjelRow[]>(() => {
    ...
    return PLAN_ENTRIES.map(entry => {
      ...
      if (entry.multiGJ) {
        const r = entry.gj === 'Risovac Krupa' ? O66_RK : O66_GJ
        actual = {
          cTrupci:  Math.round(raw66.cTrupci  * r),
          dzgo:     Math.round(raw66.dzgo     * r),
          lTrupci:  Math.round(raw66.lTrupci  * r),
          cijepano: Math.round(raw66.cijepano * r),
          ukupno:   Math.round(raw66.ukupno   * r),
        }
      } else {
  ...
  const totals = useMemo(() => ({
    ...
    actDz:   filteredRows.reduce((s,r)=>s+r.actual.dzgo,0),
    actLT:   filteredRows.reduce((s,r)=>s+r.actual.lTrupci,0),
    actCij:  filteredRows.reduce((s,r)=>s+r.actual.cijepano,0),
